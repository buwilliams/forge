...
2. **`/forge:new-project <name>`:** create a new project spec through a guided conversation. Forge reads your constitution and product spec to keep the work aligned.
3. **`/forge:start <name>`:** execute the spec. Forge determines the council, designs the pipeline, generates agents, decomposes the work into tasks, and runs them one by one.
...
When you run `/forge:start`, Forge reads the project spec and drives it through a pipeline using Claude Code subagents, each launched with precisely the context it needs and nothing more:
...
The filesystem is the source of truth. Tasks move through `todo/` → `working/` → `done/` (or `blocked/`). Runs are always resumable. Re-run `/forge:start` and Forge picks up exactly where it left off.
