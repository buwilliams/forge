...
4. **If tasks block:** Forge writes a `blocked-summary.md` in the spec directory summarizing each failure. Edit it to add context or clarify requirements, then re-run.
...
2. **Spec agent:** a subagent reads the spec, council, constitution, and product to append a Forge execution config to `project-setup.md` — global constraints, quality bar, dynamic verification approach, and execution commands. Constitution hard constraints and product requirements flow directly into these rules.
...
4. **Verifier:** a subagent generates a project-specific `verifier.md` — an independent verification agent tailored to this project's tech stack, commands, and output type.
5. **Tasks:** a subagent reads the project spec and all role agents to produce self-contained task files in `todo/`. Each task includes its role, steps, verification checks, and a save command. By the end, the spec is ready to execute.

### /forge:start executes the work

For each task in `todo/`, Forge launches a subagent carrying its role's agent file, `project-setup.md`, the task, and all council files for deliberation. The subagent implements, verifies, and commits. The project's `verifier.md` — a verification agent generated specifically for this project — then re-checks every verification item from scratch before the task is marked done.
