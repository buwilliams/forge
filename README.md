...
2. **Spec agent:** a subagent reads the spec, council, constitution, and product to append a Forge execution config to `project.md` — global constraints, quality bar, dynamic verification approach, and execution commands. Constitution hard constraints and product requirements flow directly into these rules.
...
If tasks are blocked at the end of a run, Forge writes a `blocked-summary.md` in the spec directory with each failure and its reason. Edit it and re-run to retry.
