...
2. `project.md` — the full project spec, including user design and Forge execution config (Global Constraints, tech stack commands, quality bar)
...
Read `project.md` fully. It contains two parts separated by `---`:
...
- **Comprehensive enough to stand alone** — a fresh Claude Code instance reading only this file (plus a task and `project.md`) must know exactly what to do
...
4. **Verification** — how this role verifies its work. Include the exact commands from `project.md`'s `## Execution` section (e.g., `npm run typecheck`, `npm test`, `cargo test`)
...
- Use the real test command from `project.md`'s `## Execution` section
